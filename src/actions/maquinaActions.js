...
export const getMaquinasRequest = (clienteId) => {
  ...
  if( clienteId ){
    request = axios.get(`${api.localhost.port}/maquinas/listar/minhas?cliente=${clienteId}`);
  }else{
    request = axios.get(`${api.localhost.port}/maquinas/listar`);
  }
...
export const postCadastrarMaquinaRequest = (payload) => {
  const request = axios.post(`${api.localhost.port}/maquinas/cadastrar`, payload);
